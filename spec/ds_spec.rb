...
describe DockingStation do
    ...
    describe "tests:" do
        ...
        it "should let you assign a custom capacity" do
            expect(DockingStation.new(5).capacity).to eq(5)
        end
        it "should have a default capacity of 20" do
            expect(station.capacity).to eq(20)
        end
        it "should let you see if bikes are broken or not" do
            station.dock(broken_bike)
            expect(station.view_bikes).to eq("002: Broken")
        end
        it "should accept bikes whether broken or not" do
            station.dock(bike)
            station.dock(broken_bike)
            expect(station.view_bikes).to eq("001: Working|002: Broken")
        end
        it "should only release working bikes to the user" do
            station.dock(bike)
            station.dock(broken_bike)
